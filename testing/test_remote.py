...
async def test_http(accounts):
    ...
    new_object = await client.refresh(daf.util.instance_track(it.get_object_id(accounts[0])))
    assert new_object._daf_id == accounts[0]._daf_id
    # Test account removal
    await client.remove_account(daf.util.instance_track(it.get_object_id(accounts[0])))
    await client.remove_account(daf.util.instance_track(it.get_object_id(accounts[1])))
